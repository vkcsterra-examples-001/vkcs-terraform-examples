...
# Внешний адрес для vp_db
resource "vkcs_networking_floatingip" "ext_ip_vm_db" {
  pool = "ext-net"
}
...
# NAT для vm_db
resource "vkcs_compute_floatingip_associate" "nat_vm_db" {
  floating_ip = "${vkcs_networking_floatingip.ext_ip_vm_db.address}"
  instance_id = "${vkcs_compute_instance.vm_db.id}"
  fixed_ip    = "${vkcs_compute_instance.vm_db.network[0].fixed_ip_v4}"
}
...
# порт для машины vm_db
resource "vkcs_networking_port" "port_vm_db" {
  name               = "port_vm_db"
  network_id         = "${vkcs_networking_network.new_network_01.id}"
  admin_state_up     = "true"
  security_group_ids = ["${vkcs_networking_secgroup.security_group_db.id}"]
  fixed_ip {
    subnet_id  = "${vkcs_networking_subnet.new_subnet_01.id}"
...
## группа безопасности (файерволл) для DB серверов
resource "vkcs_networking_secgroup" "security_group_db" {
  name        = "security_group_db"
}

## Правила для группы безопасности DB
# Разрешаем ssh
resource "vkcs_networking_secgroup_rule" "allow_ssh_db" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 22
  ...
  security_group_id = "${vkcs_networking_secgroup.security_group_db.id}"
}

# Разрешаем Postgres
#resource "vkcs_networking_secgroup_rule" "allow_postgres_db" {
#  direction         = "ingress"
#  ethertype         = "IPv4"
#  protocol          = "tcp"
#  port_range_min    = 5432
...
# Разрешаем icmp
resource "vkcs_networking_secgroup_rule" "allow_ping_db" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "icmp"
  remote_ip_prefix  = "0.0.0.0/0"
...
# Разрешаем http
resource "vkcs_networking_secgroup_rule" "allow_http_app" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 80
...
# Разрешаем https
resource "vkcs_networking_secgroup_rule" "allow_https_app" {
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 443
...
# Привязка к vm_db
resource "vkcs_networking_port_secgroup_associate" "assosiation_vm_db" {
  port_id = "${vkcs_networking_port.port_vm_db.id}"
  security_group_ids = [
    "${vkcs_networking_secgroup.security_group_db.id}",
  ]
}
...
# vm_db
resource "vkcs_compute_instance" "vm_db" {
  name            = "vm_db"
  image_id        = "024f4129-5fc9-43c5-8f64-946859e20e5c"
  flavor_id       = "d659fa16-c7fb-42cf-8a5e-9bcbe80a7538"
  user_data = file("cloud_init.cfg")
  network {
    port = "${vkcs_networking_port.port_vm_db.id}"
  }
}
...
# vm_db
output "vm_db" {
  value = vkcs_networking_floatingip.ext_ip_vm_db.address
}
...
## Первый инстанс
# HTTP
resource "vkcs_lb_member" "lb_vm_app_01_http" {
  address = "10.10.10.10"
...
# HTTPS
resource "vkcs_lb_member" "lb_vm_app_01_https" {
  address = "10.10.10.10"
...
## Второй инстанс
# HTTP
resource "vkcs_lb_member" "lb_vm_app_02_http" {
  address = "10.10.10.20"
  protocol_port = 80
  pool_id = "${vkcs_lb_pool.lb_pool_http.id}"
  subnet_id = "${vkcs_networking_subnet.new_subnet_01.id}"
...
# HTTPS
resource "vkcs_lb_member" "lb_vm_app_02_https" {
  address = "10.10.10.20"
  protocol_port = 443 #Порт, на котором слушает приложение на вм
  pool_id = "${vkcs_lb_pool.lb_pool_https.id}"
  subnet_id = "${vkcs_networking_subnet.new_subnet_01.id}"
...
## Третий инстанс
# HTTP
#resource "vkcs_lb_member" "lb_vm_app_03_http" {
#  address = "10.10.10.30"
#  protocol_port = 80
#  pool_id = "${vkcs_lb_pool.lb_pool_http.id}"
#  subnet_id = "${vkcs_networking_subnet.new_subnet_01.id}"
#  weight = 3 #Больше вес -- больше запросов уходит на этот хост
#}

# HTTPS
#resource "vkcs_lb_member" "lb_vm_app_03_https" {
#  address = "10.10.10.30"
#  protocol_port = 443 #Порт, на котором слушает приложение на вм
#  pool_id = "${vkcs_lb_pool.lb_pool_https.id}"
#  subnet_id = "${vkcs_networking_subnet.new_subnet_01.id}"
